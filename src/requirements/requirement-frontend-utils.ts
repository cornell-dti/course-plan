...
export function getRelatedRequirementIdsForCourseOptOut(
  ...
  const { requirementsThatDoNotAllowDoubleCounting } = getConstraintViolationsForSingleCourse(
    { uniqueId: -1 },
    requirements,
    requirementConstraintHolds(userRequirementsMap)
  );
...
export const requirementConstraintHolds = (
  userRequirementsMap: Readonly<Record<string, RequirementWithIDSourceType>>
) => <Requirement extends string>(reqA: Requirement, reqB: Requirement) =>
  // the requirements don't violate the double counting constraint violation
  allowCourseDoubleCountingBetweenRequirements(
    userRequirementsMap[reqA],
    userRequirementsMap[reqB]
  );

export const requirementConstraintHoldsAfterOptIn = (
  arbitraryOptIn: FirestoreArbitraryOptInChoices,
  userRequirementsMap: Readonly<Record<string, RequirementWithIDSourceType>>
) => <Requirement extends string>(reqA: Requirement, reqB: Requirement) =>
  // course with uniqueId is arbitrarily opted into at least one slot for both the requirements,
  // or the requirements don't violate the double counting constraint violation
  !!arbitraryOptIn[reqA]?.length ||
  !!arbitraryOptIn[reqB]?.length ||
  requirementConstraintHolds(userRequirementsMap)(reqA, reqB);
